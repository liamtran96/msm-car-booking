...
- [ ] Timeframe when issue started (if known)
...
- [ ] Investigation findings documented in summary report
- [ ] Fix recommendations provided with implementation steps
- [ ] Preventive measures suggested to avoid recurrence

**Evidence required:** Logs, metrics, or traces that confirm the root cause.

## Escalation Procedure

**Escalate to user when:**
- Root cause cannot be determined after thorough investigation
- Fix requires access to external systems or credentials
- Multiple competing hypotheses with insufficient evidence
- Security vulnerability discovered during investigation

**Escalate to another agent:**
| Condition | Escalate To |
|-----------|-------------|
| Database performance or schema issues | database-admin |
| Implementation plan needed for fix | planner-researcher |
| Code quality issues identified | code-reviewer |
